...
export default function DashboardPage() {
  ...
  const portfolio = [
    { name: 'Bitcoin', ticker: 'BTC', type: 'crypto', amount: '1.84', value: 124840, change: 4.2, icon: '₿' },
    { name: 'Ethereum', ticker: 'ETH', type: 'crypto', amount: '12.5', value: 48002, change: 1.8, icon: 'Ξ' },
    { name: 'Gold', ticker: 'XAU', type: 'metal', amount: '12 oz', value: 28101, change: -0.3, icon: '⚡' },
    { name: 'Apple Inc.', ticker: 'AAPL', type: 'stock', amount: '100 sh', value: 18940, change: 0.9, icon: '🍎' }
  ]
    ...
    <div className="max-w-7xl mx-auto px-6 py-10">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-10">
        <div>
          ...
          <h1 className="font-display font-extrabold text-3xl">{username} <span className="text-ultra-cyan">👋</span></h1>
        </div>
        <div className="flex gap-3">
          <a href="/assets" className="px-5 py-2.5 border border-ultra-border rounded-xl text-sm font-medium text-ultra-sub hover:text-ultra-text hover:border-ultra-muted transition-colors">+ Buy Assets</a>
          <a href="/wallets" className="px-5 py-2.5 bg-gradient-to-r from-ultra-cyan to-ultra-violet text-white text-sm font-semibold rounded-xl hover:opacity-90 transition-opacity">Wallets</a>
        </div>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-10">
        <div className="bg-ultra-card border border-ultra-border rounded-2xl p-6 glow-border">
          ...
          <p className="font-mono text-2xl font-bold text-ultra-text">${totalValue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
                ...
                <tr key={asset.ticker} className="border-b border-ultra-border/50 hover:bg-ultra-surface/50 transition-colors">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 rounded-xl bg-ultra-surface border border-ultra-border flex items-center justify-center text-base">{asset.icon}</div>
                      <div>
