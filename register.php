...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$username || !$email || !$password) {
        ...
        try {
            $existing = Database::findOne('users', ['$or' => [['email' => $email], ['username' => $username]]]);
            if ($existing) {
...
<div class="min-h-[80vh] flex items-center justify-center py-16 px-6">
    ...
    <div class="w-full max-w-md relative z-10">
        ...
        <div class="mb-6 px-4 py-3 rounded-xl border border-ultra-green/30 bg-ultra-green/10 text-ultra-green text-sm">
            &#10003; <?= htmlspecialchars($success) ?> <a href="/login.php" class="underline ml-2">Sign in now &rarr;</a>
        </div>
        <?php endif; ?>
        ...
        <form method="POST" class="bg-ultra-card border border-ultra-border rounded-2xl p-8 space-y-5">
            ...
            <label class="flex items-start gap-3 cursor-pointer">
                ...
                <span class="text-xs text-ultra-sub leading-relaxed">I agree to the <a href="#" class="text-ultra-cyan hover:underline">Terms</a> and <a href="#" class="text-ultra-cyan hover:underline">Privacy Policy</a>.</span>
            </label>
